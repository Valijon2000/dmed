...
# PIN va muvofiq PDF fayllar
PIN_FILES = {
    "3536": "mydoc1.pdf",
    "1234": "mydoc.pdf",
    "5678": "Xushboqov.pdf",
    "9999": "MADIYOROV ZAFARBEK G OFUR O G LI.pdf",
    "2929": "clients_list.pdf",
    "3421": "result.pdf",
    # ... xohlagancha PIN: fayl qo'sha olasiz
}
...
<head>
    ...
    {% raw %}
    <style>
        body { font-family: Arial, sans-serif; text-align: center; margin-top: 80px; background: #eef3fb; }
        .box { display:inline-block; padding:24px 32px;border-radius:12px; background:#fcfcfc;
               box-shadow:0 2px 12px #99bee244; }
...
def index():
    ...
    if request.method == "POST":
        ...
        if pin in PIN_FILES:
            session['authenticated'] = pin  # PIN bor, qaysi fayl kerakligini saqlaymiz
...
def pdf_full():
    ...
    pdf_url = url_for('static', filename=PIN_FILES[pin])
    HTML_PDF = f"""
    <!DOCTYPE html>
    <html>
    <head>
      ...
      <style>
        ...
        .viewer {{ position:fixed; inset:0; width:100%; height:100%; border:none; }}
      </style>
    </head>
    <body>
